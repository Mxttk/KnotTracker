...
class Seeding {
    ...
    private fun seedKnot() {
        // seeds the entire list of knots to users path
        val knotReference = database.child("userdata").child("productionUsers").child(User.userName.toString()).child("myKnots")

        val knotEntries = mapOf(
            "The 6 basic knots" to listOf(
                KnotData(
                    name = "Bowline",
